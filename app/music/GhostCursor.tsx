...
export const ghostCursorStudioPreset: Required<
  Pick<
    GhostCursorProps,
    | "trailLength"
    | "inertia"
...
export default function GhostCursor({
  ...
  useEffect(() => {
    ...
    fadeOpacityRef.current = ambientOpacity;
    currentMouseRef.current.set(0.5, 0.5);
    ...
    const material = new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        iPrevMouse: {
          value: trailBufRef.current.map((vector) => vector.clone()),
        },
        iOpacity: { value: 1 },
        iScale: { value: 1 },
        iBaseColor: {
      ...
      if (pointerActiveRef.current) {
        ...
        if (dt > fadeDelay) {
          ...
          fadeOpacityRef.current = Math.max(
            ambientOpacity,
            1 - k * (1 - ambientOpacity),
          );
        }
      }

      const trailLengthCount = trailBufRef.current.length;
      headRef.current = (headRef.current + 1) % trailLengthCount;
      trailBufRef.current[headRef.current].copy(mat.uniforms.iMouse.value);
      const trailUniform = mat.uniforms.iPrevMouse.value as THREE.Vector2[];
      for (let i = 0; i < trailLengthCount; i += 1) {
        const sourceIndex = (headRef.current - i + trailLengthCount) % trailLengthCount;
        trailUniform[i].copy(trailBufRef.current[sourceIndex]);
      }
  ...
  return <Host ref={containerRef} className={className} style={mergedStyle} aria-hidden />;
}
